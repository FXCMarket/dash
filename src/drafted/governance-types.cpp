...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "FxcNetwork") type = FxcNetwork;
    if(strType == "FxcNetworkVariable") type = FxcNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == FxcNetwork) s = "FxcNetwork";
    if(type == FxcNetworkVariable) s = "FxcNetworkVariable";
