...
const char * const BITCOIN_CONF_FILENAME = "fxc.conf";
const char * const BITCOIN_PID_FILENAME = "fxcd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "fxc" is a composite category enabling all Fxc-related debug output
            if(ptrCategory->count(string("fxc"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FxcCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FxcCore
    // Mac: ~/Library/Application Support/FxcCore
    // Unix: ~/.fxccore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FxcCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FxcCore";
#else
    // Unix
    return pathRet / ".fxccore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty fxc.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override fxc.conf
